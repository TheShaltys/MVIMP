...
def Conv2DNormLReLU(
    inputs, filters, kernel_size=3, strides=1, padding="VALID", Use_bias=None
):
    x = Conv2D(
    ...
    def __init__(self, inputs):
        ...
        with tf.variable_scope("G_MODEL"):
            ...
            with tf.variable_scope("b1"):
                ...
                inputs = Conv2DNormLReLU(inputs, 64)
                inputs = Separable_conv2d(inputs, 128, strides=2) + Downsample(
                    inputs, 128
                )
            ...
            with tf.variable_scope("b2"):
                ...
                inputs = Separable_conv2d(inputs, 256, strides=2) + Downsample(
                    inputs, 256
                )
            ...
            with tf.variable_scope("u1"):
                inputs = Unsample(
                    inputs, 128
                )  # The number of the filters in this layer is 128 while it is 64 in the graph of the paper. Please refer to the code.
                inputs = Conv2DNormLReLU(inputs, 64)
                inputs = Conv2DNormLReLU(inputs, 64)
