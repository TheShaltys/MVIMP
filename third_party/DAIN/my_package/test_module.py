...
def test_SeparableConvFlowModule(input1, input2, input3, filtersize):
    ...
    print(
        "CPU Forward and backward time is : "
        + str(t2 - t1)
        + "s\t"
        + str(t3 - t2)
        + "s\t"
    ...
    t3 = time.time()
    print(
        "GPU Forward and backward time is : "
        + str(t2 - t1)
        + "s\t"
        + str(t3 - t2)
        + "s\t"
    ...
    x = (
        ...
        * 2
        / (torch.abs(output_cuda) + torch.abs(output).cuda())
    )
    ...
    x = (
        (input2_cuda.grad - input2.grad.cuda())
        * 2
        / (torch.abs(input2_cuda.grad) + torch.abs(input2.grad).cuda())
    )
    y = x.cpu().data.numpy()
    x = torch.max(torch.abs(x))
    if x.cpu().data.numpy()[0] > 1e-6:
        ...
        print("pass", end="\t")
    x = (
        (input3_cuda.grad - input3.grad.cuda())
        * 2
        / (torch.abs(input3_cuda.grad) + torch.abs(input3.grad).cuda())
    )
    y = x.cpu().data.numpy()
    x = torch.max(torch.abs(x))
    if x.cpu().data.numpy()[0] > 1e-6:
...
def test_SeparableConvModule(input1, input2, input3, filtersize):
    ...
    print("Check the backward path between CPU and GPU...", end="\t")
    x = (
        (input1_cuda.grad - input1.grad.cuda())
        * 2
        / (torch.abs(input1_cuda.grad) + torch.abs(input1.grad).cuda())
    )
    y = x.cpu().data.numpy()
    x = torch.max(torch.abs(x))
    # print(x)
...
def test_WeightedFlowProjectionModule(input1, input2, input3):
    ...
    input2_cuda = Variable(input2.data.type(torch.cuda.FloatTensor), requires_grad=True)
    input3_cuda = Variable(input3.data.type(torch.cuda.FloatTensor), requires_grad=True)
    Project = WeightedFlowProjectionModule(
        threshold=20.0 / 255.0, requires_grad=True
    )  # regnenerate
    t1 = time.time()
