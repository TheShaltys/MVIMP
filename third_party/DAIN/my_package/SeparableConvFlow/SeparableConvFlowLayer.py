...
class SeparableConvFlowLayer(Function):
    ...
    def forward(self, input1, input2, input3):
        ...
        flow_ouput = torch.zeros(
            intBatches, 2, intOutputHeight, intOutputWidth
        )  # as a byproduct of SepConv, but no
        ...
        self.input2 = (
            input2.contiguous()
        )  # TODO: Note that this is simply a shallow copy?
        ...
        if input1.is_cuda:
            ...
            err = my_lib.SeparableConvFlowLayer_gpu_forward(
    ...
    def backward(self, gradoutput):
        ...
        gradinput1 = torch.zeros(
            self.input1.size()
        )  # the input1 has zero gradient because flow backprop. nothing to gradinput1
        ...
        if self.input1.is_cuda:
            ...
            err = my_lib.SeparableConvFlowLayer_cpu_backward(
                self.input1,
                self.input2,
                self.input3,
                gradoutput,
